...
class ContextGNNBERT(EncoderBERT):
    ...
    def create_context_graph(
        ...
        if not paired_seq:
            graph["pooled_out", "edge", "seq1_elm"].edge_index = self.form_edge_idcs(
                context_mask
            )
            ...
            graph["pooled_out", "edge", "seq1_elm"].edge_index = self.form_edge_idcs(
                context_mask, token_type_ids, seq_id=0
            )

            graph["pooled_out", "edge", "seq2_elm"].edge_index = self.form_edge_idcs(
                context_mask, token_type_ids, seq_id=1
            )
